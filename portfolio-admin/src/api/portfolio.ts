...
import { request, put } from './request'
...
/** 保存全部数据（全量 PUT） */
export function savePortfolio(data: PortfolioData): Promise<{ success: boolean }> {
  if (API_BASE_URL === '/mock/api') {
    // mock 模式下打印到控制台，模拟保存
    // eslint-disable-next-line no-console
    console.log('[mock] savePortfolio payload:', data)
    return Promise.resolve({ success: true })
  }
  return put<{ success: boolean }>(ENDPOINTS.portfolio, data)
}

/** 仅保存单个模块（如基础信息），后端支持则用，否则用 savePortfolio */
export function saveModule<T>(endpoint: string, data: T): Promise<{ success: boolean }> {
  if (API_BASE_URL === '/mock/api') {
    // eslint-disable-next-line no-console
    console.log('[mock] saveModule', endpoint, data)
    return Promise.resolve({ success: true })
  }
  return put<{ success: boolean }>(endpoint, data)
}

/**
 * mock 数据源（本地开发用）
 * 真实模式下由后端 API 提供，字段名保持与本文件一致
 */
  ...
  full: {
    ...
    advantages: [
      {
        ...
        group: 'strength',
        title: '电商大促实战',
        description: '主导多轮双 11 / 618 大促会场页研发，页面稳定支撑亿级 PV，可用性 99.99%。',
    ...
    projects: [
      {
        ...
        achievements: [
          '主导双 11 主会场重构，首屏性能提升 58%，转化率提升 23%',
          '沉淀营销组件库 30+ 组件，覆盖 8 条业务线',
          '推动前端监控体系建设，线上问题定位效率提升 60%'
        ],
      ...
      },
      {
        ...
        role: '前端开发工程师',
        name: '某互联网创业公司',
        description: '从 0 到 1 搭建电商运营后台与数据看板，支撑运营团队日常投放与复盘。',
        achievements: ['独立完成运营后台 20+ 模块开发', '搭建实时数据看板，报表产出效率提升 3 倍'],
    ...
    works: [
      ...
      },
      {
        ...
        title: '实时交易数据大屏',
        subtitle: '电商经营决策可视化',
        description: '面向大促作战室的实时数据大屏，秒级刷新交易、流量与履约核心指标。',
        imageUrl: '',
        themeColor: '#2F54EB',
        metrics: [
          { label: '图表组件', value: '20+' },
      ...
      },
      {
        ...
        title: '会员营销中台',
        subtitle: '千人千面的权益触达',
        description: '会员分层运营前端方案，支持权益配置、人群圈选预览与触达效果回收。',
        imageUrl: '',
        themeColor: '#722ED1',
        metrics: [
      ...
      },
      {
        ...
        title: '跨境商城小程序',
        subtitle: '轻量流畅的购物体验',
        description: '跨境电商小程序核心购物链路，针对弱网与低端机做深度体验优化。',
        imageUrl: '',
        themeColor: '#0FA98E',
        metrics: [
